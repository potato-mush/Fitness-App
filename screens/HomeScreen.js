...
const HomeScreen = () => {
  ...
  const markWorkoutDay = () => {
    setWorkoutHistoryState((prevHistory) => {
      const newHistory = [...prevHistory];
      const todayIndex = prevHistory.findIndex((item) => item.date === today);
      if (todayIndex === -1) {
        newHistory.push({ date: today, workout: true });
      } else {
        newHistory[todayIndex].workout = true;
      }
      return newHistory;
    });
  };

  const markedDates = workoutHistory.reduce((acc, { date, workout }) => {
    if (date === today) {
      acc[date] = {
        selected: true,
      ...
      >
        ...
        <View style={styles.iconBox}>
          <View style={styles.iconCard}>
            ...
            <Text style={styles.iconCount}>5</Text>
          </View>
          ...
          <View style={styles.iconCard}>
            ...
            <Text style={styles.iconCount}>10</Text>
          </View>
        </View>
        ...
        <View style={styles.recommendationBox}>
          ...
          <TouchableOpacity style={styles.startButton} onPress={markWorkoutDay}>
